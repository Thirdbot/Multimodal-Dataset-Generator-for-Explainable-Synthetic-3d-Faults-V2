...
- Favor SEGMENTATION / LOCALIZATION questions: ask the reader to point out, outline, locate, or segment an object. A question may be about a SINGLE object or, when it fits the Evidences naturally, MORE THAN ONE -- do NOT force either; let the Evidences decide which is natural, and a simple single-object question is perfectly good. Refer to an object in whatever way fits: (1) by its COORDINATE ("the fault at [x,y]"); (2) by its SPATIAL RELATION to another ("the closure to the left of the fault at [a,b]"); (3) by a COMPARATIVE / SUPERLATIVE ATTRIBUTE, but ONLY when the Evidences give the values to compare -- steeper/gentler (dip), more-displaced (throw), larger/smaller (size/area), deeper/shallower (position), or by fluid (the gas-bearing vs the water-bearing closure). Examples: "segment the fault at [x,y]", "outline the largest closure", "which of the two faults has the greater throw -- segment it". Only name objects the Evidences actually describe, so the question stays answerable and retrievable.
