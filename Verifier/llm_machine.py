...
- PREFER SEGMENTATION / LOCALIZATION questions, and prefer MULTIPLE objects: ask the reader to point out, outline, locate, or segment an object (or several), referring to each by its coordinate and, where two objects are given, by their spatial relation ("segment the fault at [x,y] and the closure to its left", "outline the two faults near the centre and say which dips steeper"). Only name objects the Evidences actually describe, so the question stays answerable and retrievable.
