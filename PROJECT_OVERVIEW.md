...
6. **Object-absent rows are TRUE empty-mask negatives.** When no object region is built, the row splits two ways. (a) If the evidence **names a specific object** (`the fault at [x,y]`) yet nothing masked, that object *should* have segmented — a suspected blank-mask **failure**, a positive in disguise — so the row is **dropped** (`[NEG-NAMED] … -> dropped`), not shipped as a fake negative. (b) Otherwise it is a **genuine absence** (`This section is structurally quiet, with no faulting developed`, `No salt body is present`, …) → emitted as a **no-target negative**: `masks=[]`, `regions=[]`, evidence carries the plain absence sentence with **no `<SEG>`**, and the answer states the absence. Nothing is segmented because nothing is there. The old whole-scene fallback (`_default_scene_mask`, which painted a full-frame mask onto every negative) was **removed** — a full-image mask for an absent object teaches a referring-segmentation model to paint the whole section, the opposite of the intent. The `#<SEG> == len(regions)` invariant still holds (0 == 0). Absence is a first-class, natural fact for **all four classes** (fault / salt / hydrocarbon closure / onlap) via `ABSENCE_TEMPLATES` (`NaturalTransform/text_transform.py`); zero counts on those class edges now surface as prose instead of being dropped as low-value.
