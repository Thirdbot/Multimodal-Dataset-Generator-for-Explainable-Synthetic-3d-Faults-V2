...
- structured **regions** — one per mask/`<SEG>`: `object_name` (coord reference, or seismic type for a global region), `class_id`, `bbox`, `center`, and the regressable `values`.
...
6. **No-evidence rows fall back to the DEFAULT SHARED SCENE.** A featureless / section-only row (no object region built) no longer goes maskless. It's emitted with the scene image + a **whole-scene mask** — the union of every object, or a full-frame mask if there's nothing to outline (`_default_scene_mask`) — as **one global region** with its own `<SEG>`. So *every* row carries image + mask + region + `<SEG>` (the empty-mask negative is gone: 0 maskless rows, was ~142). The global region's `object_name` is the **seismic type** (`"fault"`/`"closure"`/`"salt"`), not prose — because `object_name` is a classification field (§9.4); the type is derived from the section fact's edge (e.g. `number_faults → fault`).
...
- **`object_name`** — the object's coordinate reference (`"the fault at [57.5,289]"`) for an individual object, or the bare **seismic type** for a global/section region. This *replaces* `object_id` — the internal id is never surfaced. It's the classification field.
- **`seg_idx`** — ties this region to the i-th `<SEG>`.
- **`values`** — the machine-readable ground truth `{edge: target}` the model regresses: `{"dip_deg":65.8,"throw":120}`. The same values also read as words in the evidence text (retrievable) — this is the structured copy.
...
- **Segmentation** → the `masks` + `regions` columns. `regions[i]` carries `{object_name, class_id, bbox, center, values, seg_idx, mask_idx, object_type, view}` for `masks[i]`, one per `<SEG>`. The **direction this is heading** (staged, needs a QA regen): the question is a casual, multi-object, spatial *referring-segmentation* prompt ("segment the fault on the left and the closure it bounds"), the answer emits one `<SEG>` per object → mask, and such a question is verified by **retrievability** (if its `RETRIEVAL_QUERY` retrieves the referred object's facts/mask it's valid — no NLI claim to check), while a *describe*-style answer still runs NLI. `seg_idx` is the `<SEG>`↔region link the collator uses.
