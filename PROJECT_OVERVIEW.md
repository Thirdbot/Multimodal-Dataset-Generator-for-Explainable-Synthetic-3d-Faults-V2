...
The loop aims for `QUESTION_PER_GRAPH` (**12**) passing rows, capped at `MAX_ATTEMPT` (`3×` that = 36) outer attempts — right-sized to how few facts one 2D section actually carries, so it terminates early instead of grinding a fixed 200. **`QUESTION_PER_GRAPH` also sets the dataset ceiling: max rows ≈ graphs × this** (408 new-only graphs × 12 = ~4,896). It was trimmed 12→8 once for speed, then restored to 12 because 8 caps the new-only corpus at ~3,264 — below a 4k target. The trade-off: on **evidence-poor** scenes (one fault, one closure) the tail questions 9–12 can't ground and burn `MAX_ATTEMPT`, so a higher value lifts the ceiling but lowers rows/hr (≈330→≈210 at 5 workers). For each graph `RagWorkflow.generate_for_graph`:
...
2. **Generate questions.** `question_batch_generation` produces natural GroundVQA-style questions (no tags, no leaked values), rotated across five **facets** — *presence/featureless, count, location, orientation/geometry, relationship between two named structures* (`QUESTION_FACETS`, shuffled per batch) — but *evidence-gated*: an angle the evidence can't support is skipped. Questions may be **simple or compound**, and the prompt is deliberately **neutral between them** — it favors segmentation/localization but does *not* force multi-object; a simple single-object question is perfectly good. Two rules keep compound questions clean: **every clause must be answerable** from a specific value or a named relation (no subjective/visual-only clause like *"which is more prominent"*, *"stands out"*, *"compared to other structures"* — those have no evidence edge, so the model can only half-answer them), and when a question **relates or compares two objects it must name BOTH by coordinate** so both sides can be answered (both positions, both values). *History:* an earlier prompt **forced** "MULTIPLE objects", which made hard multi-clause questions the 1.5B couldn't fully cover; the edge/topic gates then rejected ~88% of answers and throughput collapsed — hence the neutral rewrite. Multi-object is still fed by the seed (`OBJECTS_PER_SEED`, step 1) when it fits. Each question ships a `RETRIEVAL_QUERY` = the **verbatim** Evidence line(s) it rests on.
...
### 13.1 QA pass — scope decided, run in progress

**Scope: new-only, clean regen.** QA runs over the **204 new diverse scenes** (`graphs/_new_only/`), *not* the 446 total — the old fault_complex graphs are pre-tonight-extraction and would skew balance and mix schema. The stale Jul-18 `verified_qa.jsonl` (3,635 old-schema rows, no `<SEG>`/coords) was backed up (`verified_qa.PRE_REGEN_0726.jsonl`) and truncated. Mechanics in **§8.4**.

**Run shape (5 sharded workers, resume-capable — §8.4).**

| knob | value | why |
|---|---|---|
| workers | 5 (`seismic-qa-0..4`) | RAM cap: ~1.7 GB each on the 15 GB box |
| `QUESTION_PER_GRAPH` | 12 | ceiling = 408 × 12 ≈ **4,896** (clears a 4k target; 8 would cap at ~3,264) |
| `CANDIDATE_PER_QUESTION` | 3 | trimmed from 5 for throughput |
| sglang context | 4096 | richer evidence overflowed 2048 (§8.1) |
| NLI | CPU, `OMP_NUM_THREADS=2` | GPU is full of sglang; don't oversubscribe cores |

**Measured throughput:** ~30 rows/hr (1 worker) → **~330 rows/hr at 5 workers, Q=8**; **~210 rows/hr at Q=12** (evidence-poor scenes reject tail questions 9–12). So ~1,200–2,000 rows in a 6 h session; a ~4k target is reached across **~2 resumed sessions** (`qa_shards.sh resume`), not one 6 h block. NLI-on-CPU + 5-worker RAM cap are the hard walls — 4k in a single 6 h is not reachable on this hardware without degrading quality.

**Quality fixes landed this pass** (all keep the gates): cross-type mismatch **25%→0%** (topic + count guards, §8.3); compound questions no longer half-answered (neutral single/compound prompt + both-objects-named + no subjective clauses, §8.2); comparatives answered as confident grounded inferences (geologist framing, §8.2); `<SEG>` moved off the LLM answer into DatasetMaker's evidence regions (§8.1/§9).
