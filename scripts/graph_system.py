...
class ArrayRelationsGraph:
    def __init__(self):
        self.graph = nx.MultiDiGraph()
        self.traces_path = None

    def build(self, trace_path):
        self._add_trace(Path(trace_path))
        self.traces_path = Path(trace_path)
        return self.graph
    ...
    def save_to_json(self, output_path=None):
        ...
        if output_path is None:
            sub_folder = self.traces_path.parent / "array_relation_graph"
            sub_folder.mkdir(parents=True, exist_ok=True)
            output_path = sub_folder / f"{self.traces_path.stem}.json"
        else:
    ...
    def _add_trace(self, trace_path):
        ...
        sample_node = f"sample:{sample_name}"
        ...
        self.graph.add_node(
            ...
            candidate_relation_count=data.get("candidate_relation_count"),
        )

        for volume in data.get("catalog", []):
            volume_node = self._volume_node(sample_name, volume)
            self.graph.add_node(
                volume_node,
                label="ArrayVolume",
                path=volume.get("path"),
                role=volume.get("role"),
                object_type=volume.get("object_type"),
            )
            self.graph.add_edge(sample_node, volume_node, type="HAS_ARRAY_VOLUME")

        for index, relation in enumerate(data.get("relations", []), start=1):
            if relation.get("status") != "ok":
                continue

            source_node = self._volume_node(sample_name, relation["source"])
            target_node = self._volume_node(sample_name, relation["target"])
            relation_node = f"array_relation:{sample_name}:{index}"
            ...
            self.graph.add_node(
                ...
                metric_family=relation.get("metric_family"),
                status=relation.get("status"),
                source_shape=relation.get("source_shape"),
                target_shape=relation.get("target_shape"),
                metrics=relation.get("metrics", {}),
            )
            self.graph.add_edge(source_node, relation_node, type="SOURCE_OF")
            self.graph.add_edge(relation_node, target_node, type="TARGET_OF")
            self.graph.add_edge(
                source_node,
                target_node,
                type=relation.get("relation_type"),
                relation_id=relation_node,
                metric_family=relation.get("metric_family"),
                source_shape=relation.get("source_shape"),
                target_shape=relation.get("target_shape"),
                metrics=relation.get("metrics", {}),
    ...
    @staticmethod
    def _volume_node(sample_name, volume):
        return f"array_volume:{sample_name}:{volume['path']}"
...
if __name__ == "__main__":
    ...
    array_trace_sample_path = sorted(traces_path.glob("*_array_relations.json"))
    if array_trace_sample_path:
        selected_array_sample = array_trace_sample_path[0]
        array_graph = ArrayRelationsGraph()
        array_graph.build(selected_array_sample)
        output_path = array_graph.save_to_json()
        print(array_graph.summary())
        print(f"saved graph to {output_path}")
